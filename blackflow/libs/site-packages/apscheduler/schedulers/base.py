...
class BaseScheduler(six.with_metaclass(ABCMeta)):
    ...
    def _configure(self, config):
        ...
        self.timezone = astimezone(config.pop('timezone', None))
    ...
    def _create_plugin_instance(self, type_, alias, constructor_kwargs):
        ...
        # Modified by alivinco . Work around over plugin system.

        from apscheduler.triggers.interval import IntervalTrigger
        from apscheduler.triggers.cron import CronTrigger
        from apscheduler.triggers.date import DateTrigger

        class_container, base_class = {
            'trigger': ({"interval":IntervalTrigger,"cron":CronTrigger,"data":DateTrigger}, BaseTrigger),
            'jobstore': ( self._jobstore_classes, BaseJobStore),
            'executor': ( self._executor_classes, BaseExecutor)
        }[type_]

        try:
            plugin_cls = class_container[alias]
        except KeyError:
            if alias in class_container:
                plugin_cls = class_container[alias]
